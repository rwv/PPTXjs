...
import { getVerticalMargins, getPregraphDir, getHorizontalAlign, getPregraphMargn } from "../layout";
...
export function genTextBody(
    ...
    for (var i = 0; i < apNode.length; i++) {
        ...
        text += "<div style='display: flex;" + sld_prg_width + sld_prg_height + "' class='slide-prgrph " + getHorizontalAlign(pNode, textBodyNode, idx, type, prg_dir, warpObj) + " " +
            prg_dir + " " + cssName + "' >";
        var buText_ary = genBuChar(pNode, i, spNode, textBodyNode, pFontStyle, idx, type, warpObj, slideFactor, fontSizeFactor);
        ...
        if (rNode === undefined && pNode !== undefined) {
            // without r
            var prgr_text = genSpanElement(pNode, undefined, spNode, textBodyNode, pFontStyle, slideLayoutSpNode, idx, type, 1, warpObj, isBullate, styleTable, isFirstBr, rtlLangsArray, slideFactor, fontSizeFactor);
            if (isBullate) {
            ...
            for (var j = 0; j < rNode.length; j++) {
                var prgr_text = genSpanElement(rNode[j], j, pNode, textBodyNode, pFontStyle, slideLayoutSpNode, idx, type, rNode.length, warpObj, isBullate, styleTable, isFirstBr, rtlLangsArray, slideFactor, fontSizeFactor);
                if (isBullate) {
                    var txt_obj = $(prgr_text)
                        .css({ 'position': 'absolute', 'float': 'left', 'white-space': 'nowrap', 'visibility': 'hidden'})
                        .appendTo($('body'));
