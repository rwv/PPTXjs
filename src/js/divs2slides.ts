...
interface Divs2SlidesSettings {
  ...
  background: false | string /** false or color*/;
  transition:
    | "default"
    | "slid"
    | "fade"
...
export function registerDivs2Slides() {
  (function ($) {
    ...
    const pptxjslideObj = {
          ...
          if (data.showTotalSlideNum) {
            $("#" + divId + " .slides-toolbar").prepend(
              $("<span></span>")
                ...
                .html(data.totalSlides.toString())
            );
          }
      ...
      gotoSlide: function (idx: number) {
        ...
        if (slides[index]) {
          ...
          $("#slides-slide-num").html(idx.toString());
        }
        return this;
      },
      ...
      startAutoSlide: function () {
        ...
        if (!isAutoSlideMode && isAutoSlideOption !== false) {
          ...
          const t = isAutoSlideOption + data.transitionTime;
    ...
    ($.fn as any).divs2slides = function (options: Divs2SlidesSettings) {
      ...
      const settings: Divs2SlidesSettings = $.extend(
        true,
        {
