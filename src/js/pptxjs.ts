        ...
        function genTextBody(textBodyNode: any, spNode: any, slideLayoutSpNode: any, slideMasterSpNode: any, type: any, idx: any, warpObj: any, tbl_col_width: any) {
            ...
            for (var i = 0; i < apNode.length; i++) {
                ...
                if (rNode === undefined && pNode !== undefined) {
                    // without r
                    var prgr_text = genSpanElement(pNode, undefined, spNode, textBodyNode, pFontStyle, slideLayoutSpNode, idx, type, 1, warpObj, isBullate, styleTable, {value: is_first_br}, rtl_langs_array, slideFactor, fontSizeFactor);
                    if (isBullate) {
                    ...
                    // with multi r
                    var isFirstBrObj = {value: is_first_br};
                    for (var j = 0; j < rNode.length; j++) {
                        var prgr_text = genSpanElement(rNode[j], j, pNode, textBodyNode, pFontStyle, slideLayoutSpNode, idx, type, rNode.length, warpObj, isBullate, styleTable, isFirstBrObj, rtl_langs_array, slideFactor, fontSizeFactor);
                        if (isBullate) {
                            var txt_obj = $(prgr_text)
                                .css({ 'position': 'absolute', 'float': 'left', 'white-space': 'nowrap', 'visibility': 'hidden'})
                                .appendTo($('body'));
                    ...
                    is_first_br = isFirstBrObj.value;
                }
